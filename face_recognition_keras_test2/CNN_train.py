...
class Dataset(object):
    ...
    def read(self, img_rows=IMAGE_SIZE, img_cols=IMAGE_SIZE, img_channels=3, nb_classes=2):
        images, labels = extract_data('./training_data')
        #print('images shape:',images.shape)
        #print('labels shape:',labels.shape)
        #labels = np.reshape(labels, [-1])
        # 训练集、测试集划分
        # allow_nd = True表示允许输入超过2维的数据
        X_train, X_test, y_train, y_test = train_test_split(images, labels, test_size=0.3, random_state=random.randint(0, 100), allow_nd = True)
        X_valid, X_test, y_valid, y_test = train_test_split(images, labels, test_size=0.5, random_state=random.randint(0, 100), allow_nd = True)
        # 更改数据集的形状结构
